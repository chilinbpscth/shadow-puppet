import {loadTemplate} from './profileRig.js';
import {getCharacter, defaultCharacter} from './characters.js';
...
const canvas = document.getElementById('wholeCanvas');
...
function fit() {
  const wrap = document.getElementById('wholeWrap');
  const scale = Math.min((wrap.clientWidth - 24) / canvas.width, (wrap.clientHeight - 24) / canvas.height);
...
function syncTools() {
  ...
  for (const b of document.querySelectorAll('[data-size]')) b.setAttribute('aria-pressed', Number(b.dataset.size) === size);
}
...
function paint(p) {
  touched += strokePaint(data.data, canvas.width, canvas.height, previous, p, size, parseHexColor(color), lock, original.data, tool === 'eraser' ? 'eraser' : 'brush');
  previous = p;
  render();
}
function finish() {
...
canvas.addEventListener('pointerdown', (ev) => {
  if (!ready || drawing || !ev.isPrimary || (ev.pointerType === 'mouse' && ev.button !== 0)) return;
  ev.preventDefault();
  const p = point(ev);
  before = copy(data);
  if (tool === 'fill') {
...
for (const id of ['brush', 'fill', 'eraser'])
...
for (const [label, hex] of colors) {
  ...
  b.onclick = () => {
    ...
    for (const el of b.parentNode.children) el.classList.toggle('active', el === b);
    syncTools();
  };
  document.getElementById('wholePalette').append(b);
}
...
for (const id of ['backHome', 'enterStage'])
  document.getElementById(id).onclick = async (ev) => {
    ...
    await queue;
    if (revision === savedRevision) location.href = document.getElementById(id).href;
  };
window.addEventListener('beforeunload', (ev) => {
  if (pending || revision !== savedRevision) {
    ev.preventDefault();
    ev.returnValue = '';
  }
});
...
function syncChrome() {
  ...
  if (print) {
    const ch = getCharacter(characterId);
    print.href = ch ? `./print.html?char=${encodeURIComponent(ch.id)}` : './print.html';
  }
  document.title = `畫${characterLabel}・皮影戲`;
}
...
async function applyPhotoFile(file) {
  ...
  try {
    ...
    const photoCanvas = downscalePhoto(img);
    const result = await openAlignOverlay({
      template: original,
      photoCanvas,
      host: document.body,
      characterLabel,
    ...
    message('未能套入相片：' + (e.message || e), true);
  }
}
...
document.getElementById('photoEntry').onclick = () => {
  if (!ready) return;
  if (photoChooser) photoChooser.hidden = !photoChooser.hidden;
};
...
async function init() {
  try {
    ...
    if (!project) {
      const old = await loadRig('./characters/wukong/rig.json');
      project = await updateProject(
        old.hasColored
          ? {assetVersion: 'wukong-legacy-v1', characterId: 'wukong', coloredPartIds: old.coloredPartIds}
          : {},
      );
    }
