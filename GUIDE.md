...
2026-09-17 本機重建中。以下散件／兩棍版本尚未完成四角驗收，未部署至 GitHub Pages；正式站不代表本機現有版本。

## 學生玩法

1. [首頁](https://chilinbpscth.github.io/shadow-puppet/) 選「開始創作」或「繼續作品」。
2. [畫悟空](https://chilinbpscth.github.io/shadow-puppet/color.html)：先喺未組裝嘅悟空散件上填色、畫花紋，旁邊有彩色參考；上舞台時再按關節組裝。不用填滿才演。
3. 用填色處理大面積，再用畫筆加幾筆花紋。橡皮恢復原白底、保留輪廓；本次開頁期間最多 20 步復原，身段外空劃不佔步數。5–10 分鐘只是入門設計目標，未經學生計時。
4. 每筆完成自動保存。「讓悟空上幕」會等待保存完成；失敗保留畫面，按「重試儲存」。
5. [本機舞台](http://127.0.0.1:5173/stage.html)：同一隻帶色角色上幕，散件會按關節重新組裝；目前拖下方兩支臂棍推提／畫弧控制左右手臂，武器跟持棒手走。這是肩根單點控制的暫行版本，未實現參考照片推斷的肘—膝雙孔硬連桿。放手便停止改姿勢。撥「轉身方向」向左／右換面向；關節微調可拖手腕、腳踝。
6. 編排「出發 → 遇險 → 迎戰」。可套用姿勢再修改；「保存這一格」保存當前格，「下一步」先保存再轉格。已存格可返回修改。
7. 三格完成後按「三格展示」，每格約兩秒；「下載三格圖」產生橫排 PNG 預覽，包含角色、背面棍線、作品名稱和各格標題，不含工具或教師資訊。可按預覽內的另存連結；檔案實際下載尚待驗收。
...
本 app 是觸控簡化操偶；左右撥桿轉身是數碼輔助，不代表已重現傳統完整操杆技法。沒有按住按鈕自動行路或自動揮棒的主玩法。目前人形兩棍控制肩角，腿不隨兩棍聯動，腳部需關節微調。參考照片顯示棍上肘釘及下方膝釘的雙點連接，機構重建仍待確認；不能稱現行肩根單點控制與實物相同。武器是角色道具，不另計操控棍。
...
已實玩四角未填色散件組裝及兩棍拖動樣本；悟空與沙僧各一個色區組裝一致性、三格 PNG 預覽已有畫面證據。59 項自動測試通過，但不代替四角完整瀏覽器回歸。768×1024、1024×768 曾核對頁面尺寸，完整布局尚未驗收；實體 iPad Safari、雙指、實物連桿、PNG 落盤及課堂學生流程仍未通過。
